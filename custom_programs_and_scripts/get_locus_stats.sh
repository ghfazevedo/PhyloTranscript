...
locus_stats () {
   ...
   mkdir -p logs
   echo "Geting loci statistics" >> logs/progress.txt
   touch "$output"
   csv_out_path=$(realpath "$output")
   echo gene,sequences,total_bp,mean_length,95_CI_length,min_length,max_length,median_legnth,contigs_bigger1kb >> $csv_out_path
